        ...
        <Transition enter-active-class="transition duration-700 ease-out delay-75" enter-from-class="opacity-0 translate-y-16 scale-95" enter-to-class="opacity-100 translate-y-0 scale-100">
          <div v-if="isOpen" class="relative w-full max-w-[1300px] h-full md:h-[90vh] lg:h-[85vh] bg-white md:rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col lg:flex-row border border-white/20">
            ...
            <div class="w-full lg:w-[45%] flex flex-col bg-white relative h-[55vh] lg:h-full">
               ...
               <div class="flex items-center justify-between border-b border-zinc-200 pl-8 pr-6 pt-6 pb-0 gap-4 shrink-0">
                 ...
                 <div class="flex items-center gap-2 pb-4">
                   <a v-if="item?.demoUrl" :href="item.demoUrl" target="_blank" class="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-indigo-200 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 text-xs font-bold transition-colors whitespace-nowrap">
                     <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg> Demo
                   </a>
                   <button @click="shareLink" class="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-zinc-200 bg-zinc-50 text-zinc-500 hover:bg-zinc-100 hover:text-black text-xs font-bold transition-colors whitespace-nowrap">
                     <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg> Share
                   </button>
                 </div>
               </div>
               ...
               <div class="flex-1 overflow-y-auto p-6 md:p-8 no-scrollbar bg-[#fafafa]">
                   <div v-if="activeTab === 'overview'" class="animate-in fade-in slide-in-from-bottom-2">
                     <h2 class="text-3xl lg:text-4xl font-extrabold tracking-tight text-black mb-2">{{ item?.title }}</h2>
                     <p class="text-xs font-semibold text-zinc-400 uppercase tracking-widest mb-4">{{ displayCategories }}</p>

                     <!-- Tech Stack Badges -->
                     <div v-if="item?.techStack" class="flex flex-wrap gap-2 mb-5">
                       <span v-for="tech in (Array.isArray(item.techStack) ? item.techStack : item.techStack.split(',')).map((t:string)=>t.trim())" :key="tech"
                         class="px-2.5 py-1 rounded-full text-[11px] font-bold border"
                         :class="tech.toLowerCase().includes('gsap') ? 'bg-green-50 text-green-700 border-green-200' : tech.toLowerCase().includes('three') ? 'bg-blue-50 text-blue-700 border-blue-200' : tech.toLowerCase().includes('react') ? 'bg-cyan-50 text-cyan-700 border-cyan-200' : tech.toLowerCase().includes('vue') ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-zinc-100 text-zinc-600 border-zinc-200'"
                       >
                         {{ tech }}
                       </span>
                     </div>

                     <p class="text-zinc-600 leading-relaxed font-medium mb-6">{{ item?.description || 'A highly polished digital asset providing production-ready code.' }}</p>
                     ...
                     <div class="bg-zinc-50 border border-zinc-200 rounded-2xl p-5 mb-6">
                       ...
                       <ul class="space-y-2">
                         <li class="flex items-center gap-2.5 text-sm text-zinc-700 font-medium"><svg class="w-4 h-4 text-emerald-500 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><polyline points="20 6 9 17 4 12"/></svg>React, Vue & HTML source code</li>
                         <li class="flex items-center gap-2.5 text-sm text-zinc-700 font-medium"><svg class="w-4 h-4 text-emerald-500 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><polyline points="20 6 9 17 4 12"/></svg>Lifetime updates & access</li>
                         <li class="flex items-center gap-2.5 text-sm text-zinc-700 font-medium"><svg class="w-4 h-4 text-emerald-500 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><polyline points="20 6 9 17 4 12"/></svg>Copy-paste ready, zero config</li>
                         <li v-if="item?.dependencies" class="flex items-center gap-2.5 text-sm text-zinc-700 font-medium"><svg class="w-4 h-4 text-emerald-500 shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><polyline points="20 6 9 17 4 12"/></svg>Npm install command included</li>
                       </ul>
                     </div>
